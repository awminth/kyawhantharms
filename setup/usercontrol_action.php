...
if($action == 'save'){
    ...
    $result = insertData_Fun("tbluser",$data);
    if($result){
        save_log($_SESSION["kyawhantharms_username"]." သည် user အားအသစ်သွင်းသွားသည်။");
        echo 1;
    }else{
        echo 0;
    }
...
if($action == 'edit'){
    ...
    $result = updateData_Fun("tbluser",$data,$where);
    if($result){
        save_log($_SESSION["kyawhantharms_username"]." သည် user အား update လုပ်သွားသည်။");
        echo 1;
    }else{
        echo 0;
    }
...
if($action == 'delete'){
    ...
    $result = deleteData_Fun("tbluser",$where);
    if($result){
        save_log($_SESSION["kyawhantharms_username"]." သည် user အားဖျက်သွားသည်။");
        echo 1;
    }else{
        echo 0;
    }    
...
if($action == "go_permission"){
    $aid = $_POST["aid"];
    $username = $_POST["username"];
    $_SESSION["go_permission_aid"] = $aid;
    $_SESSION["go_permission_name"] = $username;
    echo 1;
}

if($action == "change_siteaddress"){
    $aid = $_POST["aid"];
    $name = $_POST["name"];
    $address = $_POST["address"];
    $phno = $_POST["phno"];
    $sql = 'update tblsiteaddress set Name="'.$name.'",
    Address="'.$address.'",PhoneNo="'.$phno.'" where AID="'.$aid.'"';
    if(mysqli_query($con,$sql)){
        $_SESSION["kyawhantharms_site_aid"] = $aid;  
        $_SESSION["kyawhantharms_site_name"] = $name;                 
        $_SESSION["kyawhantharms_site_address"] = $address;
        $_SESSION["kyawhantharms_site_phno"] = $phno;
        echo 1;
    }else{
        echo 0;
    }
...
if($action == "change_password"){
    ...
    $password = $_POST["password"];
    $sql = "update tbluser set Password='{$password}' where AID='{$aid}'";
    if(mysqli_query($con,$sql)){
        $_SESSION['kyawhantharms_userpassword'] = $password;
        save_log($_SESSION['kyawhantharms_username']." Password change သွားသည်");
        echo 1;
    }else{
        echo 0;
    }
